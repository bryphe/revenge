...
let collectMeshes: (Drawable.t, Mat4.t) => list(Mesh.t) =
  (drawable, world) => {
    let rec f: (Drawable.t, Mat4.t) => list(Mesh.t) =
      (drawable, world) => {
        open Mesh;
...
let draw: (Drawable.t, Camera.t) => unit =
  (drawable, camera) => {
    let meshes = collectMeshes(drawable, Mat4.create());
    /* let passes: list(RenderPass.t) = [RenderPass.AmbientColor(Colors.Yellow)]; */

    let pass =
      RenderPass.AmbientLight(
        RenderPass.AmbientLightPass.create(~color=Colors.yellow, ~camera, ()),
      );
